### This file creates protobuf records needed for training object detection model per
### instructions of tensorflow object detection. Requires background images in folder called background 
### in the same directory contaning large (random) images for context.
...
def main(_):
    ...
    for parent, __, files in os.walk('./Data'):
        ...
        print('*' * 10 , class_name)
        with open('pbtx.txt', 'a') as f:
            st = """item {{
        id: {}
        display_name: "{}"
        name: "{}"
}}""".format(index, class_name, class_name)
            print(st,file=f)
        index += 1
        images, boxes =  augmentImage.augment(parent, './background', num=200)
